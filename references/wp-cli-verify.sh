...
if ! command -v wp >/dev/null 2>&1; then
  echo "ERROR: wp-cli not found in PATH — run this script on the destination server" >&2
  exit 1
fi
...
if [ "${POSTS_WITH_SEO:-0}" -lt "${PUBLISHED:-0}" ]; then
